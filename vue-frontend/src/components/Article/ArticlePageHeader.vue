    ...
    <div class="article-page-header">
        ...
        <Row style="display: flex; justify-content: center; align-items: flex-start">
            ...
            <p class="operate_info">
                <span class="iconfont icon-liulan" style="color: #d4237a"><b style="margin-left: 5px">{{ article.readCount
                }}</b></span>
                <span class="iconfont icon-dianzan" style="margin-left: 5px; color: #d81e06"><b style="margin-left: 5px">{{
                    article.starCount }}</b></span>
                <Tag v-if="userInfo.username != article.username" style="cursor: pointer"
                    :color="favorites == '收藏' ? 'success' : 'warning'" @click.native="addFavorites(favorites)">{{ favorites
                    }}</Tag>
                <span @click="update(article.id)" v-if="userInfo.username === article.username"><a>编辑</a></span>
            </p>
            </Col>
        </Row>
    </div>
