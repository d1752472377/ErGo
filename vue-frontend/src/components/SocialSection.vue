<template>
    <div class="social-section">
        ...
        <div v-if="type == '2'" style="text-align: center; margin-top: 40px">
            ...
            <Button icon="logo-usd" type="warning" size="large" style="margin-left: 80px"
                @click="$router.push('/recharge')">金币充值</Button>
        </div>
        <div class="likes">
            <vue-star-plus v-model="active" class="i-star__component">
                <a class="i-star__picture thumbs" slot="icon" @click="thumbs">
                    <img :src="require(active
        ...
        <div class="comment">
            <comment avatar="https://ss2.bdstatic.com/70cFvnSh_Q1YnxGkpoWK1HF6hhy/it/u=3987769337,1868985049&fm=26&gp=0.jpg"
                commentWidth="100%" placeholder="想说点什么呢" :commentNum="commentNum" :commentList="commentList" label="评论者"
                @doSend="doSend" @doChidSend="doChidSend"></comment>
        </div>
    </div>
</template>
